...
def MSMLA50(input_shape):
    ...
    # Multi-scale layer
    x0 = Conv2D(depth[0], (5, 5), padding='same', kernel_initializer='he_normal')(inputs)
    x1 = Conv2D(depth[1], (3, 3), padding='same', kernel_initializer='he_normal')(inputs)
    x2 = Conv2D(depth[0], (1, 1), padding='same', kernel_initializer='he_normal')(inputs)
    ...
    # X = Dense(depth[2], activation='relu', name='dense1', kernel_initializer='he_normal',kernel_regularizer=tensorflow.keras.regularizers.l2(1e-2))(X)
    # X = Dropout(rate=0.5)(X)
    #
    # X = Dense(depth[2], activation='relu', name='dense2', kernel_initializer='he_normal',kernel_regularizer=tensorflow.keras.regularizers.l2(1e-2))(X)
    # X = Dropout(rate=0.5)(X)
    # FC layer for LCZ classification
    X = Dense(17, activation='softmax', name='fc' + str(17), kernel_initializer='he_normal')(X)
